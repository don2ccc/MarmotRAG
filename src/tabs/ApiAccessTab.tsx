...
import { Key, Plus, Copy, Trash2, Zap, CalendarClock, Activity, ToggleLeft, ToggleRight, Code2, BookOpen } from "lucide-react";
...
export default function ApiAccessTab({ userId, sources, showToast, showConfirm }: Props) {
  ...
  const [docsTab, setDocsTab] = useState<"retrieve" | "sources">("retrieve");
              ...
              {keys.map(k => (
                <tr key={k.id} className="border-b border-white/5 last:border-0 hover:bg-white/5">
                  ...
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => revokeKey(k.id)} className="p-1.5 text-white/55 hover:text-red-400 cursor-pointer" title="Revoke">
        ...
        <div className="fixed inset-0 z-[70] bg-black/80 flex items-center justify-center" onClick={() => setSecret(null)}>
          <div onClick={e => e.stopPropagation()} className="bg-[#131720] border border-[#86C9A4]/40 rounded-xl p-6 w-full max-w-lg mx-4 space-y-4 shadow-2xl">
            ...
            <div className="flex items-center gap-2 bg-black border border-white/10 rounded-lg p-3">
              ...
              <button onClick={copySecret} className="p-2 text-white/50 hover:text-[#86C9A4] cursor-pointer" title="Copy">
      ...
      <div className="bg-[#131720] border border-white/10 rounded-xl overflow-hidden">
        <div className="flex items-center border-b border-white/10">
          ...
          <div className="ml-auto flex">
            {(["retrieve", "sources"] as const).map(t => (
              <button
                key={t}
                onClick={() => setDocsTab(t)}
                className={`px-4 py-3 font-mono text-[10px] uppercase tracking-wider font-bold border-l border-white/10 cursor-pointer ${
                  docsTab === t ? "text-[#86C9A4] bg-[#86C9A4]/5" : "text-white/55 hover:text-white/70"
                }`}
              >
                {t === "retrieve" ? "POST /retrieve" : "GET /sources"}
              </button>
            ))}
          </div>
        </div>
          ...
          {docsTab === "retrieve" ? (
            <>
              <div>
                ...
                <pre className="bg-black border border-white/10 rounded-lg p-4 text-[11px] font-mono text-[#86C9A4] overflow-x-auto">
...
X-API-Key: mrmk_...
{
  ...
  "sourceFilter": []            // optional
              ...
              <div>
                ...
                <pre className="bg-black border border-white/10 rounded-lg p-4 text-[11px] font-mono text-white/70 overflow-x-auto">
{`{
  "query": "...",
  "chunks": [
    {
  ...
  "latencyMs": 245
              ...
              <p className="text-[11px] text-white/55 leading-relaxed">
                The key can only retrieve chunks from <span className="text-white/70">your own documents plus platform-shared documents</span>.
...
{`{
  "sources": [
    { "id": "src-...", "name": "...", "type": "...",
      "vectorsCount": 537, "lastSync": "...",
      "isShared": false, "ownerName": "Jane Doe" }
  ]
}`}
                </pre>
              </div>
